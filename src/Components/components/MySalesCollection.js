...
import { withdrewRewards, transferedNFT, updateListed, fetchNfts, fetchSales } from '../../GlobalState/User';
...
const MyNftCollection = ({ showLoadMore = true, walletAddress = null}) => {
    ...
    // mySalesFetching
    // mySalesNfts
    const isLoading = useSelector((state) => state.user.mySalesFetching);
    const nfts = useSelector((state) => user.mySalesNfts);
    ...
    useEffect(async() => {
        dispatch(fetchSales(user.address));
    }, []);
